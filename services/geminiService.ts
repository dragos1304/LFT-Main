...
import type { StudySet, GradedAnswer, ChatMessage, PracticeQuestion, StudySourceType, OutlineNode, ProcessableFile, Flashcard } from '../src/types';
...
export const generateFlashcardForConcept = async (concept: string): Promise<Pick<Flashcard, 'frontText' | 'backText'>> => {
    const prompt = `You are a subject-matter expert. Create one high-quality, concise Q/A flashcard for the concept: "${concept}". The response must be a single, valid JSON object that strictly adheres to the provided schema. Do not include any markdown formatting like \`\`\`json.`;
    
    const response = await generateAndParse(prompt, singleFlashcardSchema);

    return {
        frontText: response.front_text,
        backText: response.back_text,
    };
};
